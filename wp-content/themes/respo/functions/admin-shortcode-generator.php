...
-----------------------------------------------------------------------------------*/
	...
	function init() {
		...
		if ( ( current_user_can( 'edit_posts' ) || current_user_can( 'edit_pages' ) ) && get_user_option( 'rich_editing') == 'true' )  {
			...
			wp_register_script( 'tmnf-colourpicker', $this->framework_url() . 'js/colorpicker.js', array( 'jquery' ), '3.6', true ); // Loaded into the footer.
			wp_enqueue_script( 'tmnf-colourpicker' );
			
			// Register the colourpicker CSS.
			wp_register_style( 'tmnf-colourpicker', $this->framework_url() . 'css/colorpicker.css' );
			wp_enqueue_style( 'tmnf-colourpicker' );
			
			// Register the custom CSS styles.
			wp_register_style( 'tmnf-shortcode-generator', $this->framework_url() . 'css/shortcode-generator.css' );
...
-----------------------------------------------------------------------------------*/
	...
	function filter_mce_external_plugins( $plugins ) {
        ...
        $plugins['ThemnificThemesShortcodes'] = $this->framework_url() . 'js/shortcode-generator/editor_plugin.js';
