...
if (!function_exists( "tmnf_remove_wpautop")) {
	function tmnf_remove_wpautop($content) {
...
function tmnf_shortcode_box($atts, $content = null) {
   extract(shortcode_atts(array(	'type' => 'normal',
   	...
   	return '<div class="tmnf-sc-box '.$type.' '.$size.' '.$style.' '.$border.'"'.$custom.'>' . do_shortcode( tmnf_remove_wpautop($content) ) . '</div>';
}
...
function tmnf_shortcode_button($atts, $content = null) {
	...
	if ( $window )
   	...
   	$output = '<a '.$window.'href="'.$link.'" class="tmnf-sc-button'.$class_output.'" '.$color_output.'><span class="tmnf-'.$style.'">' . tmnf_remove_wpautop($content) . '</span></a>';
   	return $output;
}
...
function tmnf_shortcode_twocol_one($atts, $content = null) {
   return '<div class="twocol-one">' . tmnf_remove_wpautop($content) . '</div>';
}
add_shortcode( 'twocol_one', 'tmnf_shortcode_twocol_one' );

function tmnf_shortcode_twocol_one_last($atts, $content = null) {
   return '<div class="twocol-one last">' . tmnf_remove_wpautop($content) . '</div>';
}
...
function tmnf_shortcode_quote($atts, $content = null) {
   ...
   return '<div class="tmnf-sc-quote' . $class . '"><p>' . tmnf_remove_wpautop($content) . '</p></div>';
}
...
function tmnf_shortcode_ilink($atts, $content = null) {
   ...
   return '<span class="tmnf-sc-ilink"><a class="'.$style.'" href="'.$url.'" '.$custom_icon.'>' . tmnf_remove_wpautop($content) . '</a></span>';
}
...
<a name="fb_share" type="'.$type.'" share_url="'.$url.'">' . tmnf_remove_wpautop($content) . '</a>
...
function tmnf_shortcode_contactform ( $atts, $content = null ) {
		...
		if ( $email == '' ) {} else {
			...
			$html .= '<p><label for="contactName">' . __( 'Name','themnific') . '</label>' . "\n";
			...
			$html .= '<p><label for="contactEmail">' . __( 'Email','themnific') . '</label>' . "\n";
